...
var SelectQtComponent = function SelectQtComponent() {
  ...
  // We're only installing one component, deselecting will cause everything else to be uninstalled.
  // If this gets run on systems that need more Qt stuff installed, this logic should be adjusted to
  // just uninstall older msvc components.
  beforeDiskSpace = installer.requiredDiskSpace();
  widget.deselectAll();
  widget.selectComponent(componentId);
  selectedNewComponent = (beforeDiskSpace !== installer.requiredDiskSpace());
  return {
    'selectedNewComponent': selectedNewComponent,
...
Controller.prototype.ComponentSelectionPageCallback = function ComponentSelectionPageCallback() {
  ...
  var qt5Path;
  // Check archive so deselection works
  CheckCategory(gui, 'LTS', false);
  CheckCategory(gui, 'Archive', true);
  ...
  try {
    ...
    if (typeof this.installedVersion !== 'undefined') {
      if (this.installedVersion.selectedNewComponent) {
        ...
        // Installer won't let us do anything if component selections don't change.
...
Controller.prototype.ReadyForInstallationPageCallback = function ReadyForInstallPageCallback() {
  console.log('ReadyForInstallPageCallback');
  gui.clickButton(buttons.CommitButton);
};
...
Controller.prototype.PerformInstallationPageCallback = function PerformInstallationPageCallback() {
  console.log('In perform installation callback');
  var page = gui.currentPageWidget();
  page.setAutomatedPageSwitchEnabled(true);
};
